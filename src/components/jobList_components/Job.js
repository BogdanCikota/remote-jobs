...
function Job({job}) {
    ...
    return (
        <Link onClick={e=>{window.screen.width < 640 ? dispatch(setJobPositionTop(e.target.offsetTop - 40))  : dispatch(setJobPositionTop(e.target.offsetTop - 100)) }} className={` job border-b  hover:bg-gray-50 p-4 block xl:p-4 xl:pl-6 lg:flex justify-between `} to={"/JobDescription"} state={job}>
